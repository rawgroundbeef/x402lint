...
gaps:
  - truth: "Browser bundle is under 15KB minified"
    ...
    artifacts:
      - path: "packages/x402lint/dist/index.iife.js"
...
|----------|----------|--------|---------|
| `packages/x402lint/dist/index.js` | ESM bundle | VERIFIED (59,239 bytes) | Exists, substantive, wired via `package.json exports["."].import.default` and `module` field |
| `packages/x402lint/dist/index.cjs` | CJS bundle | VERIFIED (59,902 bytes) | Exists, substantive, wired via `package.json exports["."].require.default` and `main` field |
| `packages/x402lint/dist/index.d.ts` | TypeScript declarations (ESM) | VERIFIED (15,391 bytes, 483 lines) | Exports all public types and functions. Wired via `exports["."].import.types` and top-level `types` field |
| `packages/x402lint/dist/index.d.cts` | TypeScript declarations (CJS) | VERIFIED (15,392 bytes, 483 lines) | Mirrors ESM types. Wired via `exports["."].require.types` |
| `packages/x402lint/dist/index.iife.js` | IIFE browser bundle (minified) | PARTIAL (26,781 bytes) | Exists, substantive, correctly exposes `x402Lint` global. File is IIFE not UMD (functionally equivalent). Size exceeds 15KB target. |
| `packages/x402lint/dist/index.d.mts` | ESM-specific TypeScript declarations | N/A | Not generated. Not needed: package uses `"type": "module"` so `.d.ts` serves as ESM declarations. publint validates without this file. |
| `packages/x402lint/tsdown.config.ts` | Multi-format build configuration | VERIFIED (30 lines) | Array config with two entries: ESM+CJS (neutral, dts, sourcemaps) and IIFE (browser, minified, globalName: x402Lint) |
| `packages/x402lint/package.json` | Package metadata with correct exports | VERIFIED (55 lines) | Split exports with per-condition types, `files: ["dist/"]`, `sideEffects: false`, `prepublishOnly` script, `publint` in devDependencies |
...
| `tsdown.config.ts globalName` | `window.x402Lint` | IIFE bundle global assignment | WIRED | Config has `globalName: 'x402Lint'`, IIFE opens with `var x402Lint=(function(e){...` |
...
| BUILD-03: Browser bundle exposes `window.x402Lint` with `{ validate, detect, normalize }` | SATISFIED | Verified via vm.runInContext -- all three are callable functions |
...
**Test:** Open a TypeScript file that imports from `x402lint`, hover over `validate()` call and its return value
...
**Test:** Create an HTML file with `<script src="dist/index.iife.js"></script>` and call `window.x402Lint.validate(...)` in a subsequent script
**Expected:** `x402Lint` is available on `window`, `validate()` returns a proper result object
