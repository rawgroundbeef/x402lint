...
must_haves:
  truths:
    - "ESM import works: import { validate } from 'x402lint' resolves and validate is a function"
    - "CJS require works: const { validate } = require('x402lint') resolves and validate is a function"
    - "UMD bundle exposes window.x402Lint with validate, detect, normalize as callable functions"
    - "Browser bundle (UMD) is under 15KB minified"
    - "TypeScript consumers get full type inference on validate() return type"
  artifacts:
    - path: "packages/x402lint/dist/index.mjs"
      provides: "ESM bundle"
    - path: "packages/x402lint/dist/index.cjs"
      provides: "CJS bundle"
    - path: "packages/x402lint/dist/index.d.ts"
      provides: "TypeScript declarations"
    - path: "packages/x402lint/dist/index.d.mts"
      provides: "ESM-specific TypeScript declarations"
    - path: "packages/x402lint/dist/index.d.cts"
      provides: "CJS-specific TypeScript declarations"
    - path: "packages/x402lint/dist/index.umd.js"
      provides: "UMD browser bundle (minified)"
    - path: "packages/x402lint/tsdown.config.ts"
  ...
  key_links:
    - from: "packages/x402lint/package.json exports.'.'.import"
      to: "packages/x402lint/dist/index.mjs"
      via: "Node.js module resolution"
      pattern: '"import":\\s*"\\./dist/index\\.mjs"'
    - from: "packages/x402lint/package.json exports.'.'.require"
      to: "packages/x402lint/dist/index.cjs"
      via: "Node.js module resolution"
      pattern: '"require":\\s*"\\./dist/index\\.cjs"'
    - from: "packages/x402lint/package.json exports.'.'.types"
      to: "packages/x402lint/dist/index.d.ts"
      via: "TypeScript module resolution"
      pattern: '"types":\\s*"\\./dist/index\\.d\\.ts"'
    - from: "packages/x402lint/tsdown.config.ts UMD globalName"
      to: "window.x402Lint"
      via: "UMD bundle global assignment"
      pattern: "globalName:\\s*'x402Lint'"
---
...
<task type="auto">
  ...
  <action>
    ...
    **package.json** - Update to match build outputs and add publish-readiness:
    ...
    9. Add "repository", "homepage", "bugs" fields:
       - "repository": { "type": "git", "url": "https://github.com/anthropics/x402lint" }
  ...
  <verify>
    Run `cat packages/x402lint/tsdown.config.ts` to confirm multi-format array config with ESM/CJS and UMD entries.
    Run `cat packages/x402lint/package.json` to confirm:
...
<task type="auto">
  ...
  <files>
    packages/x402lint/dist/ (build output, not committed)
  </files>
  <action>
    1. Install dependencies: Run `pnpm install` from repo root to install publint.

    2. Run the build: Execute `pnpm --filter x402lint build` from repo root.
    ...
    5. Verify ESM import works:
       ...
       ```
       echo 'import { validate, detect, normalize, VERSION } from "./packages/x402lint/dist/index.mjs"; console.log("ESM OK:", typeof validate, typeof detect, typeof normalize, VERSION);' > /tmp/test-esm.mjs
    ...
    6. Verify CJS require works:
       ```
       echo 'const { validate, detect, normalize, VERSION } = require("./packages/x402lint/dist/index.cjs"); console.log("CJS OK:", typeof validate, typeof detect, typeof normalize, VERSION);' > /tmp/test-cjs.cjs
    ...
    7. Verify UMD bundle works in a pseudo-browser context:
       ```
       echo 'globalThis.window = globalThis; require("./packages/x402lint/dist/index.umd.js"); console.log("UMD OK:", typeof window.x402Lint.validate, typeof window.x402Lint.detect, typeof window.x402Lint.normalize);' > /tmp/test-umd.cjs
    ...
    9. Run publint to validate package configuration:
       `cd packages/x402lint && npx publint`
  ...
  <verify>
    ...
    - `pnpm --filter x402lint build` exits 0
    ...
    - `pnpm --filter x402lint test` passes all existing tests
...
1. "import { validate } from 'x402lint' works in ESM" -- Verified by Task 2 step 5 (ESM import test)
2. "const { validate } = require('x402lint') works in CJS" -- Verified by Task 2 step 6 (CJS require test)
3. "Loading UMD bundle makes window.x402Lint.validate callable" -- Verified by Task 2 step 7 (UMD test)
...
- UMD bundle assigns validate, detect, normalize to window.x402Lint
