...
files_modified:
  ...
  - packages/x402check/src/types/.gitkeep
  - packages/x402check/src/detection/.gitkeep
  - packages/x402check/src/validation/.gitkeep
...
must_haves:
  truths:
    - "Running pnpm install at repo root installs workspace dependencies for both root and packages/x402check/"
    - "The existing website (index.html) still loads and functions correctly after restructuring"
    - "packages/x402check/src/index.ts exists and compiles with tsc --noEmit"
    - "pnpm --filter x402check test runs vitest and the smoke test passes"
  artifacts:
    ...
    - path: "packages/x402check/package.json"
      ...
      contains: "x402check"
    - path: "packages/x402check/tsconfig.json"
    ...
    - path: "packages/x402check/test/index.test.ts"
      provides: "SDK smoke test"
      contains: "describe"
    - path: "packages/x402check/vitest.config.ts"
      provides: "Test runner configuration"
      contains: "defineConfig"
  key_links:
    - from: "packages/x402check/tsconfig.json"
      to: "packages/config/typescript/base.json"
      via: "extends field"
      pattern: "extends.*config/typescript/base"
    - from: "package.json"
      to: "packages/x402check"
      via: "pnpm --filter proxy scripts"
      pattern: "pnpm --filter x402check"
...
<objective>
Restructure the repository into a pnpm workspace monorepo with three workspace members: the existing website (moved to apps/website/), the SDK package skeleton (packages/x402check/), and a shared config package (packages/config/). The existing website must still load and function correctly after the move.
...
<task type="auto">
  ...
  <action>
    ...
    6. Create `apps/website/package.json`:
       ```json
       {
         "name": "@x402check/website",
         "version": "0.0.0",
         "private": true,
         "description": "x402check validation website"
       }
       ```
...
<task type="auto">
  ...
  <action>
    ...
    3. Create `packages/x402check/package.json`:
       ```json
       {
         "name": "x402check",
         "version": "0.0.1",
    ...
    4. Create `packages/x402check/tsconfig.json`:
       ```json
       {
    ...
    5. Create `packages/x402check/vitest.config.ts`:
       ```typescript
    ...
    6. Create `packages/x402check/tsdown.config.ts`:
       ```typescript
    ...
    7. Create directory structure for future phase code:
       - `packages/x402check/src/types/` (Phase 6)
       - `packages/x402check/src/detection/` (Phase 6)
       - `packages/x402check/src/validation/` (Phase 8)
       - `packages/x402check/src/crypto/` (Phase 7)
       Add `.gitkeep` in each empty directory so git tracks them.

    8. Create `packages/x402check/src/index.ts`:
       ```typescript
    ...
    9. Create `packages/x402check/test/index.test.ts`:
       ```typescript
    ...
    10. Install devDependencies for the SDK package:
        ```bash
        cd packages/x402check
        pnpm add -D typescript vitest tsdown
        ```
        Then run `pnpm install` from repo root to update the lockfile.

    11. Verify everything works:
        - `pnpm --filter x402check lint` (tsc --noEmit) should succeed
        - `pnpm --filter x402check test` (vitest run) should pass
  </action>
  <verify>
    - `pnpm --filter x402check lint` exits 0 (TypeScript compiles)
    - `pnpm --filter x402check test` exits 0 (vitest smoke test passes)
    - `cat packages/x402check/tsconfig.json` shows extends pointing to shared config
    - `ls packages/x402check/src/types packages/x402check/src/detection packages/x402check/src/validation packages/x402check/src/crypto` confirms all subdirectories exist
...
<task type="auto">
  ...
  <action>
    ...
    2. Verify workspace members are recognized:
       ...
       pnpm ls --depth -1 -r
       ```
       Must show at least: root (x402check-monorepo), @x402check/website, @x402check/config, x402check.

    3. Verify proxy scripts work:
       - `pnpm run test:sdk` from repo root should pass (delegates to packages/x402check)
...
1. **pnpm install works**: Running `pnpm install` at the repo root installs workspace dependencies for both root and `packages/x402check/`
...
3. **SDK compiles**: `packages/x402check/src/index.ts` exists and compiles with `tsc --noEmit`
   - Verify: `pnpm --filter x402check lint` exits 0 (runs tsc --noEmit)
</verification>

<success_criteria>
- pnpm workspace with 4 members (root, website, config, x402check) is functional
- Website files moved to apps/website/ with git history preserved
- SDK package at packages/x402check/ with TypeScript strict config, vitest, tsdown
