...
must_haves:
  ...
  artifacts:
    - path: "packages/x402check/src/crypto/c32check.ts"
      provides: "c32check decode wrapper (matches base58.ts pattern)"
      contains: "decodeC32Address"
    - path: "packages/x402check/src/validation/stacks-address.ts"
      provides: "Stacks address validation function"
      exports: ["validateStacksAddress"]
    - path: "packages/x402check/src/registries/networks.ts"
      provides: "Stacks network entries in KNOWN_NETWORKS"
      contains: "stacks:1"
    - path: "packages/x402check/test/validation/stacks-address.test.ts"
      provides: "Comprehensive Stacks address validation tests"
      min_lines: 80
  key_links:
    - from: "packages/x402check/src/validation/address.ts"
      to: "packages/x402check/src/validation/stacks-address.ts"
      via: "case 'stacks' in switch dispatch"
      pattern: "case 'stacks'"
    - from: "packages/x402check/src/validation/stacks-address.ts"
      to: "packages/x402check/src/crypto/c32check.ts"
      via: "import decodeC32Address"
      pattern: "decodeC32Address"
    - from: "packages/x402check/src/registries/networks.ts"
      to: "stacks:1"
...
<objective>
Add Stacks blockchain address validation to x402check, covering SP/SM (mainnet) and ST/SN (testnet) address prefixes with c32check checksum verification, network mismatch detection, and actionable error messages.
...
<task type="auto">
  ...
  <verify>
    - `pnpm ls c32check --filter x402check` shows c32check installed
    - `pnpm tsc --noEmit --project packages/x402check` passes (no type errors)
    - No build errors: `pnpm build --filter x402check` succeeds
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    5. Run full test suite and build:
       - `pnpm test --filter x402check` -- all tests pass
       - `pnpm build --filter x402check` -- build succeeds
       - Verify IIFE bundle size: `ls -la packages/x402check/dist/index.global.js` -- should be under 45KB
  </action>
  <verify>
    - `pnpm test --filter x402check` passes all tests (existing + new)
    - `pnpm build --filter x402check` succeeds
    - IIFE bundle `packages/x402check/dist/index.global.js` is under 45KB
    - `pnpm tsc --noEmit --project packages/x402check` -- no type errors
  </verify>
  <done>
...
<verification>
1. `pnpm test --filter x402check` -- all tests pass (existing + new Stacks tests)
2. `pnpm build --filter x402check` -- clean build, no errors
3. `ls -la packages/x402check/dist/index.global.js` -- under 45KB minified
4. `pnpm tsc --noEmit --project packages/x402check` -- no TypeScript errors
</verification>
