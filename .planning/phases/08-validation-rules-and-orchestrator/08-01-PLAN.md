...
Output: 7 files in `packages/x402check/src/validation/rules/` -- 6 rule modules + 1 barrel export. Also updates errors.ts with 2 new error codes.
</objective>
...
<task type="auto">
  ...
  <action>
**FIRST: Update `packages/x402check/src/types/errors.ts`** to add two new error codes.
...
**Then create four files in `packages/x402check/src/validation/rules/`:**
...
```
  ...
  <verify>
Run `cd /Users/rawgroundbeef/Projects/x402check && npx tsc --noEmit -p packages/x402check/tsconfig.json` -- no type errors. Verify all 7 files exist: `ls packages/x402check/src/validation/rules/`. Verify new error codes: `grep -n 'INVALID_TIMEOUT\|INVALID_URL' packages/x402check/src/types/errors.ts`.
  </verify>
  <done>
...
3. `grep 'INVALID_TIMEOUT' packages/x402check/src/types/errors.ts` shows the new error code
4. `grep 'INVALID_URL' packages/x402check/src/types/errors.ts` shows the new error code
