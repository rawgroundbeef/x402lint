...
must_haves:
  ...
  key_links:
    - from: "packages/x402lint/src/types/manifest.ts"
...
<task type="auto">
  ...
  <verify>
    Run `pnpm --filter x402lint exec tsc --noEmit` to confirm TypeScript compiles without errors.
...
<task type="auto">
  ...
  <verify>
    Run `pnpm --filter x402lint exec tsc --noEmit` -- must pass with zero errors.
    Run `pnpm --filter x402lint test` -- all existing 217 tests must still pass (no regressions).
...
<verification>
1. `pnpm --filter x402lint exec tsc --noEmit` passes (zero errors)
2. `pnpm --filter x402lint test` passes (all 217 existing tests, zero regressions)
3. `ManifestConfig`, `ServiceMetadata`, `ServiceContact`, `ManifestValidationResult` types are importable from `x402lint`
4. `isManifestConfig` function is importable from `x402lint`
