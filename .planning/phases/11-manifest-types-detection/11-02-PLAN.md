...
must_haves:
  ...
  artifacts:
    - path: "packages/x402check/src/detection/wild-manifest.ts"
      provides: "normalizeWildManifest(), generateStableEndpointId()"
      min_lines: 60
    - path: "packages/x402check/test/manifest.test.ts"
      provides: "Manifest detection, guard, and wild normalization tests"
      min_lines: 100
    - path: "packages/x402check/test/detection.test.ts"
      provides: "Extended detection tests including manifest format"
      contains: "manifest"
  key_links:
    - from: "packages/x402check/src/detection/wild-manifest.ts"
      to: "packages/x402check/src/types/manifest.ts"
    ...
    - from: "packages/x402check/test/manifest.test.ts"
      to: "packages/x402check/src/index.ts"
...
<task type="auto">
  ...
  <verify>
    Run `pnpm --filter x402check exec tsc --noEmit` -- must compile cleanly.
    Run `pnpm --filter x402check test` -- existing tests must still pass.
  </verify>
  <done>
...
<task type="auto">
  ...
  <verify>
    Run `pnpm --filter x402check test` -- ALL tests must pass, both new manifest tests and existing 217 tests.
    Run `pnpm --filter x402check test -- --reporter=verbose 2>&1 | grep -c "PASS\|✓"` to count total passing tests. Should be 217 + new manifest tests.
  </verify>
  <done>
...
<verification>
1. `pnpm --filter x402check exec tsc --noEmit` passes
2. `pnpm --filter x402check test` passes (all old + new tests)
