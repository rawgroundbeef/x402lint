...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/x402lint/src/crypto/index.ts"
  ...
  key_links:
    - from: "packages/x402lint/src/crypto/eip55.ts"
      to: "packages/x402lint/src/crypto/keccak256.ts"
      via: "import { keccak256 }"
      pattern: "import.*keccak256.*from.*keccak256"
    - from: "packages/x402lint/src/crypto/keccak256.ts"
      to: "@noble/hashes/sha3"
      via: "import { keccak_256 }"
      pattern: "import.*keccak_256.*from.*@noble/hashes"
    - from: "packages/x402lint/src/crypto/base58.ts"
      to: "@scure/base"
      via: "import { base58 }"
...
<task type="auto">
  ...
  <action>
    1. Install `@noble/hashes` and `@scure/base` as devDependencies in packages/x402lint:
       ```
       cd packages/x402lint && pnpm add -D @noble/hashes @scure/base
       ```

    2. Remove `packages/x402lint/src/crypto/.gitkeep` (directory will have real files).

    3. Create `packages/x402lint/src/crypto/keccak256.ts`:
    ...
    4. Create `packages/x402lint/src/crypto/base58.ts`:
    ...
    5. Create `packages/x402lint/src/crypto/eip55.ts`:
    ...
    6. Create `packages/x402lint/src/crypto/index.ts`:
       ```typescript
    ...
    7. Add `NO_EVM_CHECKSUM` to `packages/x402lint/src/types/errors.ts`:
  ...
  <verify>
    Run `cd packages/x402lint && pnpm exec tsc --noEmit` — no TypeScript errors.
...
<task type="auto">
  ...
  <action>
    1. Create directory `packages/x402lint/test/crypto/` if it doesn't exist.

    2. Create `packages/x402lint/test/crypto/keccak256.test.ts`:
       ```
       describe('keccak256', () => {
    ...
    3. Create `packages/x402lint/test/crypto/base58.test.ts`:
       ```
    ...
    4. Create `packages/x402lint/test/crypto/eip55.test.ts`:
       ```
    ...
    Run tests with `cd packages/x402lint && pnpm test` and ensure all pass. If any test fails due to wrong expected value (e.g., empty string Base58 behavior), fix the test expectation to match the actual correct behavior while documenting why.
  </action>
  <verify>
    Run `cd packages/x402lint && pnpm test` — ALL tests pass (existing + new crypto tests).
...
<verification>
1. `cd packages/x402lint && pnpm exec tsc --noEmit` — zero TypeScript errors
2. `cd packages/x402lint && pnpm test` — all tests pass (existing Phase 6 tests + new crypto tests)
3. `grep '"@noble/hashes"' packages/x402lint/package.json` — appears in devDependencies
4. `grep '"@scure/base"' packages/x402lint/package.json` — appears in devDependencies
5. `grep 'NO_EVM_CHECKSUM' packages/x402lint/src/types/errors.ts` — error code exists
6. `grep '"dependencies"' packages/x402lint/package.json` — should NOT exist (zero runtime deps)
</verification>
