...
must_haves:
  ...
  key_links:
    - from: "packages/x402lint/src/validation/evm-address.ts"
      to: "packages/x402lint/src/crypto/eip55.ts"
      via: "import { toChecksumAddress, isValidChecksum }"
      pattern: "import.*from.*crypto"
    - from: "packages/x402lint/src/validation/solana-address.ts"
      to: "packages/x402lint/src/crypto/base58.ts"
    ...
    - from: "packages/x402lint/src/validation/address.ts"
      to: "packages/x402lint/src/validation/evm-address.ts"
      via: "import { validateEvmAddress }"
      pattern: "import.*validateEvmAddress"
    - from: "packages/x402lint/src/validation/address.ts"
      to: "packages/x402lint/src/validation/solana-address.ts"
...
<task type="auto">
  ...
  <action>
    1. Remove `packages/x402lint/src/validation/.gitkeep` (directory will have real files).

    2. Create `packages/x402lint/src/validation/evm-address.ts`:
    ...
    3. Create `packages/x402lint/src/validation/solana-address.ts`:
    ...
    4. Create `packages/x402lint/src/validation/address.ts`:
    ...
    5. Create `packages/x402lint/src/validation/index.ts`:
       ```typescript
    ...
    6. Update `packages/x402lint/src/index.ts`:
...
<task type="auto">
  ...
  <action>
    1. Create directory `packages/x402lint/test/validation/` if it doesn't exist.

    2. Create `packages/x402lint/test/validation/evm-address.test.ts`:
    ...
    3. Create `packages/x402lint/test/validation/solana-address.test.ts`:
    ...
    4. Create `packages/x402lint/test/validation/address.test.ts`:
    ...
    Run tests with `cd packages/x402lint && pnpm test` and ensure all pass.
  </action>
  <verify>
    Run `cd packages/x402lint && pnpm test` — ALL tests pass (existing Phase 6 tests + crypto tests + validation tests).
...
2. `cd packages/x402lint && pnpm test` — all tests pass (Phase 6 + Phase 7)
